...
define('DEFAULT_LOG', '/Users/Guilhaume/Documents/Development/Sites/guilhaume.fr/log/messages.log');
...
function write_log($message, $logfile='') {
  ...
  if($logfile == '') {
    ...
    // the constant is not defined and there is no log file given as input
        ...
        return ['status' => false, 'message' => 'No log file defined!'];
    }
  }
  ...
  if($fd = fopen($logfile, "a")) {
    $result = fputcsv($fd, array($date, $remote_addr, $request_uri, var_export($message, true)));
    fclose($fd);

    if($result > 0)
      return ['status' => true];
    else
      return ['status' => false, 'message' => 'Unable to write to '.$logfile.'!'];
  }
  else {
    return ['status' => false, 'message' => 'Unable to open log '.$logfile.'!'];
  }
}
